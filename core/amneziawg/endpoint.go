...
func NewEndpoint(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options Options) (adapter.Endpoint, error) {
	...
	keepalive := fmt.Sprint(peer.PersistentKeepaliveInterval)
	var deviceParameters strings.Builder
	for _, line := range strings.Split(parameters, "\n") {
		if strings.HasPrefix(line, "persistent_keepalive_interval=") {
			keepalive = strings.TrimPrefix(line, "persistent_keepalive_interval=")
		} else if line != "" {
			deviceParameters.WriteString(line + "\n")
		}
	}
	parameters = deviceParameters.String()
	...
	ipc += "persistent_keepalive_interval=" + keepalive + "\n"
	...
	// AWG adds its own padding on top of WireGuard's overhead. The outer
	// datagram can exceed the physical MTU even with a 1280-byte inner MTU.
	// Match the upstream AWG UDP bind: allow IP fragmentation by default.
	// An explicit udp_fragment=false still wins in sing-box's dialer.
