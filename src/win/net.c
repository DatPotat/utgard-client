                 ...
                 wchar_t *err, size_t errcap)
{
    ...
    for (;;) {
        ...
        /* sing-box's archive is 33 MB; a server that keeps sending past this
           is not sending what we asked for, and would fill the disk. */
        total += got;
        if (total > NET_DOWNLOAD_MAX) {
            fail(err, errcap, L"Файл больше ожидаемого — загрузка прервана");
            goto done;
        }
