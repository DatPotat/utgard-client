                         ...
                         int *added, int *kept, wchar_t *err, size_t errcap)
{
    ...
    text[0] = '\0';
    /* The file is rewritten below from what was read: a partial read would
       cut the user's own lines off. */
    if (file_read(path, text, EXC_TEXT_MAX, NULL) == FILE_READ_PARTIAL) {
        if (err && errcap)
            StringCchCopyW(err, errcap, L"ipset-exclude-user.txt больше 64 КБ — "
                                        L"изменять его автоматически программа не будет");
        return 0;
    }
                       ...
                       wchar_t *err, size_t errcap)
{
    ...
    /* Into our own folder, not %TEMP%: we run elevated, and a file we create
       in a folder any program of the user can write to is a file that
       program can point elsewhere first. */
    if (!singbox_root(root, ZAPRET_PATH_MAX) ||
        FAILED(StringCchPrintfW(temp_path, temp_cap, L"%slist\\zapret-hosts.txt", root))) {
