...
static LRESULT CALLBACK wnd_proc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_APP_INSTALL_ASK:
        /* sing-box first; AmneziaWG next, once sing-box is in place - right
           away if it already is, or when its download finishes. */
        if (!offer_install(hwnd) && singbox_present() && !g_installing)
            offer_awg_install(hwnd, 0);
        return 0;
    ...
    case WM_APP_INSTALL: {
        ...
        }
        {
            ...
            free(job);
            layout(hwnd);
            if (after_singbox) offer_awg_install(hwnd, 0);
        }
        return 0;
    }
