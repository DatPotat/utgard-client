...
static void paint_utgard(HDC dc, const RECT *c)
{
    ...
    /* Title and subtitle (38 px together) centred between the tab line
       above and the separator at +74, the button likewise. */
    dot(dc, PAD + S(6), top + S(32), S(5), g_vpn_on ? CLR_OK : CLR_MUTED);
    text_at(dc, PAD + S(22), top + S(18), w, S(22),
            g_vpn_on ? L"VPN включён" : L"VPN выключен",
            g_vpn_on ? CLR_OK : CLR_MUTED, g_font_big, DT_LEFT);
    {
        wchar_t sub[320];
        if (g_vpn_on && g_awg_lost) {
            StringCchCopyW(sub, 320, L"AmneziaWG: соединение потеряно — выключите и включите VPN");
        } else if (g_vpn_on && g_prof.active >= 0 && g_prof.active < g_prof.count) {
            char    tag[96];
            wchar_t wtag[128];
            genconf_tag(&g_prof, g_prof.active, tag, sizeof tag);
            to_wide(tag, wtag, 128);
            StringCchPrintfW(sub, 320,
                             g_switch_note ? L"Переключение не удалось — работает прежний профиль: %s"
                                           : L"через %s",
                             wtag);
        } else if (g_busy && g_busy_text) {
            StringCchCopyW(sub, 320, g_busy_text);
        } else if (g_installing) {
            StringCchCopyW(sub, 320, g_installing == 2 ? L"Загрузка AmneziaWG…"
                                                       : L"Загрузка sing-box…");
        ...
        text_at(dc, PAD + S(22), top + S(38), w, S(18), sub,
                g_vpn_on && (g_awg_lost || g_switch_note) ? CLR_WARN : CLR_MUTED, g_font_small, DT_LEFT);
    }
    ...
    if (g_sub_busy) {
        ...
        text_at(dc, PAD, top + S(162), w, S(20),
                L"Проверка задержек…", CLR_ACCENT, g_font_small, DT_RIGHT);
    } else if (g_prof.count) {
        wchar_t hint[96];
        int k, icmp_silent = 0;
        /* WireGuard and AmneziaWG are pinged over ICMP, which many servers
           drop: say that a silent one is not a dead tunnel. */
        for (k = 0; k < g_prof.count && k < PROFILES_MAX; k++)
            if (g_prof.items[k].link.proto == LINK_WG && g_ping[k] == -1) icmp_silent = 1;
        if (icmp_silent)
            StringCchCopyW(hint, 96, L"ICMP не отвечает — это не значит, что туннель не работает");
        else
            StringCchPrintfW(hint, 96, L"всего: %d · двойной клик — выбрать", g_prof.count);
...
static void paint_settings(HDC dc, const RECT *c)
{
    ...
    text_at(dc, PAD, top + S(468), w, S(20),
            L"Используемые версии — нажмите, чтобы открыть страницу релиза:",
            CLR_MUTED, g_font_small, DT_LEFT);
}
...
void draw_profile(const DRAWITEMSTRUCT *d)
{
...

    {
        ...
        StringCchPrintfW(right, 128, active ? L"%s · %s · выбран" : L"%s · %s",
                         proto_label(&e->link), server);
    }
    icmp_silent = e->link.proto == LINK_WG && g_ping[d->itemID] == -1;
    text_at(d->hDC, r.left, r.top, r.right - r.left - (icmp_silent ? S(128) : S(84)), r.bottom - r.top,
...

    {
        ...
        else if (ms < 0)   { StringCchCopyW(lat, 24, icmp_silent ? L"ICMP не отвечает" : L"нет");
                             color = CLR_WARN; }
        else {
